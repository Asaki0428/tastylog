...
router.get("/regist/:shopId(\\d+)", async( req, res, next) => {
  ...
  res.cookie("_csrf", token); //tokenの発行
  ...
  try{
    ...
    shopName = shop.name;
    review = {};
    res.render("./accont/reviews/regist-form.ejs", { shopId, shopName, review });
  }catch(err){
    next(err);
  }
});
...
router.post("/regist/confirm", (req, res, next) => {
  ...
  if(error){
    res.render("./acccount/reviews/regist-form.ejs", { error, shopId, shopName, review });
    return;
  } //errorのチェック
...
router.post("/regist/execute", async(req, res, next) => {
  ...
  try{
    transaction = MySQLClient.beginTransaction();
    transaction.executeQuery(
      await sql("SELECT_SHOP_BY_ID_UPDATE"),
      [shopId]
    );
    transaction.executeQuery(
